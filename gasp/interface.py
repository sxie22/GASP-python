...
class Interface():
    '''
    Contains methods required to match the 2D lattices (no c lattice vector)
    of two different cells.
    '''

    match_parameters = parameters['LatticeMatch']
        ...
        if found:
            ...
            if write_all_matches:
                ...
                for i in range(n_matches):
                    matches_dict['Match'+str(i)] = uv_all[i]
                    # TODO: Make the dictionary more clean.
                with open('matches_dict.out', 'a') as f:
                    species_1 = [str(i) for i in iface1.species]
                    species_2 = [str(i) for i in iface2.species]
                    f.write('Lattice matching is done using following inputs: \n')
                    f.write('species #1: \n{0} \nlattice #1: \n{1} \n'.format(
                                                               species_1, ab1
                                                               ))
                    f.write('species #2: \n{0} \nlattice #2: \n{1} \n'.format(
                    ...
                    f.write('Parameters: \n')
                    f.write('Max area:  {0} \nMax mismatch:  {1}\n'.format(
                                                    max_area, max_mismatch
                                                    ))
                    f.write('Max angle difference: {0} \narea ratio tol(r1r2): {1}\n\n'.format(
